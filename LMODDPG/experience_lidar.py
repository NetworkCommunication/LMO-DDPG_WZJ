...
flag=0
class EXPERIENCELIDARAgent(object):
    ...
    def train_model(self):
        ...
        not_good = self.check_batch(batch)
        num = len(not_good) - 1
        for j in not_good:
            if not_good[j] != -1:
                np.delete(batch, j, 0)
        need =self.batch_size-num
        if need != 0:
            batch2 = random.sample(self.memory, need)
    ...
    def replay_buffer_success(self):

        batch1 = random.sample(self.memory, self.batch_size)
        batch1 = np.array(batch1)
        for i in batch1:


            np.append(batch1,self.memory.__getitem__("goal1"))

        return batch1

    def replay_buffer_fail(self):

        batch2=np.empty((1,5))
        for i in range(self.memory_size):

            np.append(batch2, self.memory.__getitem__("collision"))

        batch2=sorted(batch2,key=abs(batch2[4]))
        batch2=batch2[::-1]
        ...
        if flag !=1:
            ...
            while True:
                try:
                    ...
                    if col == -1:
                        array = [-5.13, 3, 0]
                        observe, reward, done, info = env.step(array)
                        col = env.lidar()
                        arrv.append([-5.13, 3, 0])
                    ...
                    while not done and timestep < time_limit:
                        ...
                        arrp.append(observe[2])
                        p=int(p)+int(Env.comsum)
                        c=int(c)+int(Env.whole_collsion)
                        image = observe[0]
                        vel = observe[1]
                        try:
                            image = transform_input(image, args.img_height, args.img_width)
                    ...
                    y = str(y)
                    t=str(t)
                    f1=open('save_memory/'  +agent_name+ '/experience_lidar_x.csv',mode='a')
                    f1.write(x + '\n')
                    f1.close()

                    f2 = open('save_memory/' +agent_name+ '/experience_lidar_y.csv', mode='a')
                    f2.write(y + '\n')
                    f2.close()


                    f3 = open('save_memory/' +agent_name + '/experience_lidar_t.csv', mode='a')
                    f3.write(t + '\n')
                    f3.close()

                    p=str(p)
                    c = str(c)
                    f3 = open('save_memory/' +agent_name + '/experience_lidar_p.csv', mode='a')
                    f3.write(p + '\n')
                    f3.close()

                    f3 = open('save_memory/' +agent_name + '/experience_lidar_c.csv', mode='a')
                    f3.write(c + '\n')
                    f3.close()

                    v = str(arrv)
                    f6 = open('save_memory/' + agent_name + '/experience_lidar_v.csv', mode='a')
                    f6.write(v + '\n')
                    f6.close()
                    arrv.clear()

                    position = str(arrp)
                    f7 = open('save_memory/' +agent_name + '/experience_lidar_position.csv', mode='a')
                    f7.write(position + '\n')
                    f7.close()
                    arrp.clear()
