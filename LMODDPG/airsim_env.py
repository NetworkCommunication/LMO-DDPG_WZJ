...
class Env:
    ...
    def compute_reward(self, quad_pos, quad_vel, collision_count):
        Env.whole_collsion=Env.whole_collsion+collision_count
        ...
        vd = abs(v)
        th = 0.25 * abs(0.5 * 1.293 * v * v * 0.0225 * vd - 6.3 * 9.8)
        p = 0.25 * (0.0015 * (th / 0.0122612604 + 3 * v * v) * np.sqrt(0.0128388049668874 * th) + 1.131 * th * np.sqrt(
            (np.sqrt(th * th / 0.00659350496016 + v * v * v * v / 4) - v * v / 2)))
        if vel[0]<-45.96 and vel[1]>26.8:
            pos=1
        dis_value = ahpal * math.exp(-disnew / 1000)
        Env.disini=dis
        enger=beta*p/20/3
        Env.comsum=p
        collision_value = gama * (math.exp(-Env.whole_collsion) - 1)
        stat=theata*pos
        reward = dis_value-enger-collision_value+stat
        return reward
