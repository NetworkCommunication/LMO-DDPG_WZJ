...
class LMODDPGAgent(object):
    ...
    def train_model(self):
        ...
        need=num
        if need!=0:
            batch2 = random.sample(self.memory, self.batch_size)
            not_good2 = self.check_batch(batch2)
            num2 = len(not_good2) - 1
            for j in not_good:
                if not_good2[j] != -1:
                    np.delete(batch2, j, 0)
            if self.batch_size-num2>=need:
                for i in range(0,need):
                    np.append(batch,batch2[i])
            else:
                for i in batch2:
                    np.append(batch, batch2[i])
                need-=num2
        ...
        if flag !=1:
            ...
            while True:
                try:
                    ...
                    y = str(y)
                    t=str(t)
                    f1=open('save_memory/'+agent_name + '/lmoddpg_x.csv',mode='a')
                    f1.write(x + '\n')
                    f1.close()

                    f2 = open('save_memory/'+agent_name + '/lmoddpg_y.csv', mode='a')
                    f2.write(y + '\n')
                    f2.close()

                    f3 = open('save_memory/' +agent_name+ '/lmoddpg_t.csv', mode='a')
                    f3.write(t + '\n')
                    f3.close()
                    ...
                    f4 = open('save_memory/'+agent_name + '/lmoddpg_p.csv', mode='a')
                    f4.write(p + '\n')
                    f4.close()

                    f5 = open('save_memory/' +agent_name+ '/lmoddpg_c.csv', mode='a')
                    f5.write(c + '\n')
                    f5.close()

                    v=str(arrv)
                    f6 = open('save_memory/' +agent_name+ '/lmoddpg_v.csv', mode='a')
                    f6.write(v + '\n')
                    f6.close()
                    arrv.clear()

                    position = str(arrp)
                    f7 = open('save_memory/' +agent_name+ '/lmoddpg_position.csv', mode='a')
                    f7.write(position + '\n')
                    f7.close()
                    arrp.clear()
